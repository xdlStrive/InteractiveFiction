...
router.post('/add', (req, res) => {
    CounterModel.findOneAndUpdate({ counter_id: 'volume' }, {
        $inc: { counter_num: 1 }
    }, {
        ...
        new VolumeModel({    // 新增卷
            ...
            res.json({ code: 20000, msg: '新增卷成功！', data: docs })
        })
    })
})
...
router.get('/list', (req, res) => {
    ...
    VolumeModel.find({}, { _id: 0, volume_id: 1, title: 1 }).sort({ volume_id: 1 }).exec((err, doc) => {
        try {
            if (!err && doc) {
