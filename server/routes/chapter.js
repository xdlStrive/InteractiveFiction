...
router.post('/add', (req, res) => {
    CounterModel.findOneAndUpdate({ counter_id: 'chapter' }, {
        $inc: { counter_num: 1 }
    }, {
        ...
        new ChapterModel({    // 新增章节
            ...
            res.json({ code: 20000, msg: '新增章节成功！', data: docs })
        })
    })
})
...
router.post('/modify', (req, res) => {
    ChapterModel.findOneAndUpdate({ chapter_id: req.body.chapter_id}, {
        title: req.body.title,
        $addToSet: {paragraph_list: { $each: [req.body.paragraph_list] }},
        $addToSet: {chapter_comment: { $each: [req.body.chapter_comment] }},
    })
})

// 查询文章接口
router.get('/list', (req, res) => {
    ChapterModel.find({}, { _id: 0, chapter_id: 1, title: 1 }).sort({chapter_id: 1}).exec((err, doc) => {
        try {
            if (!err && doc) {
                return res.json({ code: 20000, msg: '章节列表获取成功！', data: doc })
            }
        } catch (err) {
            res.json({ code: 1, message: '后端出错', data: err })
        }
    })
})
